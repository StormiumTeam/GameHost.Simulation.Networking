...
namespace Unity.Networking.Transport
{
    ...
    public unsafe struct DataStreamWriter : IDisposable
    {
        ...
        {
            WritePackedFloatDelta(value, 0, model);
        }
    ...
    public unsafe struct DataStreamReader
    {
        ...
        public void Flush(ref Context ctx)
        {
            ctx.m_ReadByteIndex -= (ctx.m_BitIndex >> 3);
            ctx.m_BitIndex      =  0;
            ctx.m_BitBuffer     =  0;
        ...
        public bool ReadBitBool(ref Context ctx)
        {
            FillBitBuffer(ref ctx);
            return ReadRawBitsInternal(ref ctx, 1) == 1;
        }
        ...
        uint ReadRawBitsInternal(ref Context ctx, int numbits)
        {
            ...
            return res;
                ...
                public float ReadPackedFloat(ref Context ctx, NetworkCompressionModel model)
	        {
	            return ReadPackedFloatDelta(ref ctx, 0, model);
	        }

            	        public float ReadPackedFloatDelta(ref Context ctx, float baseline, NetworkCompressionModel model)
	        {
	#if ENABLE_UNITY_COLLECTIONS_CHECKS
	            AtomicSafetyHandle.CheckReadAndThrow(m_Safety);
	#endif
	            FillBitBuffer(ref ctx);
	            if (ReadRawBitsInternal(ref ctx, 1) == 0)
	                return baseline;
        ...
        {
            //return ReadPackedInt(ref ctx, model);
