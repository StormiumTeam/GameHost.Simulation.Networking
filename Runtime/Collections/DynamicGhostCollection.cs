...
{
    public class DynamicGhostCollectionSystem : BaseGhostCollectionSystem
    {
        #region Send
                                              ...
                                              JobHandle                                 inputDeps)
        {
            ...
            var serializerSystem     = sendSystem.World.GetExistingSystem<GhostSerializerCollectionSystem>();
            var serializers          = serializerSystem.BeginSerialize(sendSystem, Allocator.TempJob);
            ...
            for (int con = 0; con < connectionStates.Count; ++con)
            {
                ...
                var serializeJob = new SerializeJob
                {
                    ...
                    ackFromEntity          = sendSystem.GetComponentDataFromEntity<NetworkSnapshotAck>(true),
                    bufferFromEntity       = sendSystem.GetBufferFromEntity<OutgoingSnapshotDataStreamBufferComponent>(),
                    serialSpawnChunks      = serialSpawnChunks,
                    entityType             = entityType,
        ...
        public override JobHandle ExecuteReceive(GhostReceiveSystem                                  receiveSystem,
                                                 ...
                                                 JobHandle                                           inputDeps)
        {
            var serializers = receiveSystem.World.GetExistingSystem<GhostSerializerCollectionSystem>().BeginDeserialize(receiveSystem, Allocator.TempJob);
            ...
            var readJob = new ReadStreamJob
            {
                ...
                snapshotAckFromEntity = receiveSystem.GetComponentDataFromEntity<NetworkSnapshotAck>(),
